...
public class BeanPropertyUtil {
	...
	/**
	 * Obtain the read method for property <em>name</em> associated with <em>instance</em>. Once the method is located, it is stored in {@link #map}using a format <code>fully.qualified.Class#name</code> so that it is obtained quicker on subsequent
	 * requests. As the map is a {@link java.util.WeakHashMap}then memory requests may cause parts of this cache to be lost.
	 * 
	 * @param name
	 * the property name
	 * @param instance
