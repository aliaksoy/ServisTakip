...
public class FolderFileDisplay {
		...
		for (int i = 0; i < listOfFiles.length; i++) {
			if (listOfFiles[i].isFile()) {
				...
				if (listOfFiles[i].getName().contains(".kee") || listOfFiles[i].getName().startsWith("Ref")
						|| listOfFiles[i].getName().startsWith("IHasProfivisingProfile") || listOfFiles[i].getName().contains(".copyarea"))
					continue;

				String name = listOfFiles[i].getName().replaceAll(".java", "");

				//System.out.println(name);
				printStartupJob(name);
				//printProducer(name);
				//	printList(name);
	...
	private static void printStartupJob(String name) {
		System.out.println("GuiConstants.putOneLevelEntityTypeData(" + name + ".class, EntityTypeData.ENTITY_PROFILE);");
